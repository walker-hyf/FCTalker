...
class VarianceAdaptor(nn.Module):
    ...
    def forward(
        ...
        if context_word_level_encodings is not None:
            x = x. + context_word_level_encodings[:x.shape[0]]
...
class ConversationalContextWordLevelEncoder(nn.Module):
    """ Conversational Context Word Level Encoder """

    def __init__(self, preprocess_config, model_config,train_config):
        super(ConversationalContextWordLevelEncoder, self).__init__()
        self.tod_bert_size = model_config["history_encoder"]["tod_bert_size"]   # 768
        self.max_history_len =  model_config["history_encoder"]["max_history_len"]
        self.encoder_hidden = model_config["transformer"]["encoder_hidden"]  # 256
        self.batch_size = train_config["optimizer"]["batch_size"]
        self.raw_data_path = preprocess_config["path"]["raw_path"]+"/"+preprocess_config["path"]["sub_dir_name"]
        self.out_dir = preprocess_config["path"]["preprocessed_path"]

    def forward(self, basenames, raw_texts, speakers, history_speakers,history_lens, max_src_len):

        context_word_level_encodings = torch.zeros(self.batch_size ,max_src_len, self.encoder_hidden)

        for basename,raw_text,batch_size_index in zip(basenames,raw_texts,range(0,self.batch_size)):
            raw_text = raw_text.strip("\n")

            tg_path = os.path.join(
                self.out_dir, "TextGrid", basename.split("_")[-1].strip("d"), "{}.TextGrid".format(basename)
            )
            ...
            try:
                ...
                words_dir = {}
                tier_words = textgrid.get_tier_by_name("words")
                for m in tier_words:
                    s, e, w = m.start_time, m.end_time, m.text
                    if (w != ""):
                        words_dir[e] = w

                phones_num_list = []  
                for key, value in words_dir.items():
                    key = key - 0.001 
                    phones_dir[key] = "*"

                phones_dir = sorted(phones_dir.items(), key=lambda e: e[0], reverse=False)

                current_num = 0  
                for item in phones_dir:
                    if (item[1] != "*"):
                        current_num += 1
                    else:
                        phones_num_list.append(current_num)
                        current_num = 0

                row_text_words_num = len(phones_num_list)

                dialog = basename.split("_")[2].strip("d")
                turn = int(basename.split("_")[0])
                max_history_len = self.max_history_len
                history_len = min(max_history_len, turn)

                history_basenames = sorted([tg_path.replace(".wav", "") for tg_path in
                                            os.listdir(os.path.join(self.raw_data_path, f"{dialog}")) if
                                            ".wav" in tg_path], key=lambda x: int(x.split("_")[0]))
                history_basenames = history_basenames[:turn][-history_len:]

                history_basenames.append(basename)  

                history_text = "[CLS] "
                for history_basename in history_basenames:
                    dialog_num = history_basename.split("_")[-1].strip("d")  
                    dialog_speaker = history_basename.split("_")[1]  

                    tg_path = os.path.join(
                        self.out_dir, "TextGrid", history_basename.split("_")[-1].strip("d"),
                        "{}.TextGrid".format(history_basename)
                    )
                ...
                with torch.no_grad():
                    ...
                    m = nn.Linear(self.tod_bert_size, self.encoder_hidden)
                    m = m.cuda()
                    input = hiddens[0]

                    # input = input.to(device)
                    output = m(input)[-row_text_words_num:]

                encodings_index = 0  
                context_word_level_encoding = torch.zeros(max_src_len, self.encoder_hidden)

                for num,phonme_num in zip(range(0, row_text_words_num),phones_num_list):
                        for i in range(0,phonme_num):
                            if (encodings_index >= max_src_len):
                                break
                            else:
                                context_word_level_encoding[encodings_index] = output[num]
                                encodings_index += 1

                context_word_level_encoding = context_word_level_encoding.unsqueeze(0)
                context_word_level_encodings[batch_size_index] = context_word_level_encoding
            except:
                continue

        return context_word_level_encodings

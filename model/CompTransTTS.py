...
from .modules import PostNet, VarianceAdaptor, ConversationalContextEncoder, ConversationalContextWordLevelEncoder
...
class CompTransTTS(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config, train_config):
        ...
        self.history_level = model_config["history_encoder"]["level_type"]
        if self.history_level != "none":
            if self.history_level == "word":
                self.context_word_level_encoder = ConversationalContextWordLevelEncoder(preprocess_config, model_config,train_config)
    ...
    def forward(
        ...
        # Context Word Level Encoding
        if self.history_level != "none":
            if self.history_level == "word":

                context_word_level_encodings = self.context_word_level_encoder(
                    basenames,    
                    raw_texts,    
                    speakers,      
                    history_speakers, 
        ...
        context_word_level_encodings = context_word_level_encodings.cuda()
